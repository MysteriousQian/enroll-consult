...
type AcceptDetail struct {
	...
	LowestScore  float64 `json:"lowest_score" gorm:"type:bigdecimal(6,2);comment:最低分数"`
	LowestRank   int     `json:"lowest_rank" gorm:"type:int unsigned;comment:最低名次"`
	HighestScore float64 `json:"highest_score" gorm:"type:bigdecimal(6,2);comment:最高分数"`
	Average      float64 `json:"average" gorm:"type:bigdecimal(6,2);comment:平均分"`
